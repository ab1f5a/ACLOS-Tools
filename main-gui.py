...
PATH_CACHE = os.path.join(CONFIG_DIR, "launcher_path.cfg")
...
class ACLOSToolsGUI:
    ...
    def _build_shell(self):
        ...
        _logo_png_path = get_resource_path("logo.png")
        self._logo_img = None
        if os.path.exists(_logo_png_path):
            try:
                from PIL import Image, ImageTk
                _pil_img = Image.open(_logo_png_path).resize((S(40), S(40)), Image.LANCZOS)
                self._logo_img = ImageTk.PhotoImage(_pil_img)
            except Exception:
                try:
                    self._logo_img = tk.PhotoImage(file=_logo_png_path)
                    self._logo_img = self._logo_img.subsample(
                        max(1, self._logo_img.width() // S(40)),
                        max(1, self._logo_img.height() // S(40))
                    )
                except Exception:
                    self._logo_img = None
        if self._logo_img:
            logo_box = tk.Label(logo_wrap, image=self._logo_img,
                                bg=COLORS['surface'], width=S(40), height=S(40))
            logo_box.pack()
        else:
        ...
        self._add_nav("cache",  "🧹  启动器工具",    self._show_cache,   locked=True)
        ...
        tk.Label(btm, text="© 2026 ab1f5a. All rights reserved.", font=(F, FS(7)),
    ...
    def _show_cache(self):  self._show_page("cache")
    ...
    def _build_cache_page(self, parent):
        F = self.F
        self._page_topbar(parent, "缓存清理")
        self._cache_panel = CacheCleanEmbedded(parent, self)
    ...
    def deploy_patcher(self):
        ...
        def deploy():
            ...
            try:
                ...
                valorant_lnk = r"C:\Users\Public\Desktop\无畏契约.lnk"
                _kernel32 = ctypes.windll.kernel32
                _exists = _kernel32.GetFileAttributesW(valorant_lnk) != 0xFFFFFFFF
                if _exists:
                    try:
                        _kernel32.SetFileAttributesW(valorant_lnk, 0x80)
                        if not _kernel32.DeleteFileW(valorant_lnk):
                            raise ctypes.WinError()
                        self.root.after(0, lambda: self.log(
                            "已删除桌面快捷方式: 无畏契约.lnk", "success"))
                    except Exception as e:
                        self.root.after(0, lambda err=e: self.log(
                            f"删除无畏契约.lnk 失败: {err}", "warning"))
                else:
                    self.root.after(0, lambda: self.log(
                        "桌面未找到无畏契约.lnk，跳过", "info"))
                ...
                self.root.after(0, lambda: self._msgbox("info", "部署成功", "绕过补丁已成功部署到桌面！"))

            except Exception as e:
                self.root.after(0, lambda: self.log(f"部署失败: {e}", "error"))
                self.root.after(0, lambda: self.set_status("部署失败", COLORS['error']))
                self.root.after(0, lambda err=e: self._msgbox("error", "部署失败", str(err)))
    ...
    def _msgbox(self, kind, title, message):
        tmp = tk.Toplevel(self.root)
        tmp.withdraw()
        if os.path.exists(self._ico_path):
            try:
                tmp.iconbitmap(self._ico_path)
            except Exception:
                pass
        if kind == "info":
...
class VCEmbedded:
    ...
    def auto_find(self):
        ...
        def find():
            p = find_asar_path()
            if p:
                from_cache = os.path.exists(PATH_CACHE)
                self.parent.after(0, lambda: self.path_var.set(p))
                self.parent.after(0, lambda: self.vc_log_msg(
                    f"{'从缓存加载' if from_cache else '定位成功'}: {p}", "success"))
            else:
...
class CacheCleanEmbedded:
    ...
    def _build(self, outer):
        ...
        card1 = tk.Frame(outer, bg=COLORS['surface'],
                         highlightbackground=COLORS['border'], highlightthickness=1)
        card1.pack(fill=tk.X, pady=(0, S(10)))
        inner1 = tk.Frame(card1, bg=COLORS['surface'])
        inner1.pack(fill=tk.X, padx=S(14), pady=S(12))

        ico1 = tk.Frame(inner1, bg=COLORS['warning_light'], width=S(38), height=S(38))
        ico1.pack(side=tk.LEFT)
        ico1.pack_propagate(False)
        tk.Label(ico1, text="🗑", font=(F, FS(17)),
                 bg=COLORS['warning_light']).place(relx=.5, rely=.5, anchor="center")

        tf1 = tk.Frame(inner1, bg=COLORS['surface'])
        tf1.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=S(12))
        tk.Label(tf1, text="工具缓存清理", font=(F, FS(10), "bold"),
                 bg=COLORS['surface'], fg=COLORS['fg']).pack(anchor="w")
        tk.Label(tf1,
                 text="删除工具自身产生的缓存文件：ACLOS Tools 数据目录及 asar 路径配置缓存",
                 font=(F, FS(8)), bg=COLORS['surface'], fg=COLORS['fg_muted'],
                 wraplength=S(380), justify=tk.LEFT).pack(anchor="w", pady=(S(2), 0))

        RoundedButton(inner1, text="立即清理",
                      command=self.clean_tool_cache,
                      bg=COLORS['warning'], fg="white",
                      hover_bg=self.main_app._darken_color(COLORS['warning'], 20),
                      font_size=FS(9), font_weight="bold",
                      width=S(86), height=S(30), radius=S(8)).pack(side=tk.RIGHT)

        detail1 = tk.Frame(card1, bg=COLORS['bg_alt'],
                           highlightbackground=COLORS['border'], highlightthickness=1)
        detail1.pack(fill=tk.X, padx=S(14), pady=(0, S(10)))
        for line in [
            f"• %APPDATA%\\ACLOS Tools\\  （工具数据目录）",
            f"• %APPDATA%\\ACLOS\\launcher_path.cfg  （路径缓存配置）",
            f"• %USERPROFILE%\\Desktop\\ACLOS.lnk  （桌面快捷方式）",
        ]:
            tk.Label(detail1, text=line, font=("Consolas", FS(7)),
                     bg=COLORS['bg_alt'], fg=COLORS['fg_muted'],
                     anchor="w", padx=S(10), pady=S(3)).pack(fill=tk.X)

        card2 = tk.Frame(outer, bg=COLORS['surface'],
                         highlightbackground=COLORS['border'], highlightthickness=1)
        card2.pack(fill=tk.X, pady=(0, S(10)))
        inner2 = tk.Frame(card2, bg=COLORS['surface'])
        inner2.pack(fill=tk.X, padx=S(14), pady=S(12))

        ico2 = tk.Frame(inner2, bg=COLORS['error_light'], width=S(38), height=S(38))
        ico2.pack(side=tk.LEFT)
        ico2.pack_propagate(False)
        tk.Label(ico2, text="♻", font=(F, FS(17)),
                 bg=COLORS['error_light']).place(relx=.5, rely=.5, anchor="center")

        tf2 = tk.Frame(inner2, bg=COLORS['surface'])
        tf2.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=S(12))
        tk.Label(tf2, text="启动器全新安装", font=(F, FS(10), "bold"),
                 bg=COLORS['surface'], fg=COLORS['fg']).pack(anchor="w")
        tk.Label(tf2,
                 text="运行卸载程序后彻底删除 ACLOS 安装目录及用户数据，以便重新安装",
                 font=(F, FS(8)), bg=COLORS['surface'], fg=COLORS['fg_muted'],
                 wraplength=S(380), justify=tk.LEFT).pack(anchor="w", pady=(S(2), 0))

        RoundedButton(inner2, text="执行操作",
                      command=self.fresh_install,
                      bg=COLORS['error'], fg="white",
                      hover_bg=self.main_app._darken_color(COLORS['error'], 20),
                      font_size=FS(9), font_weight="bold",
                      width=S(86), height=S(30), radius=S(8)).pack(side=tk.RIGHT)

        detail2 = tk.Frame(card2, bg=COLORS['bg_alt'],
                           highlightbackground=COLORS['border'], highlightthickness=1)
        detail2.pack(fill=tk.X, padx=S(14), pady=(0, S(10)))
        for line in [
            "• \\ACLOS\\uninst.exe  （执行卸载程序）",
            "• :\\ACLOS\\  （强制删除安装目录）",
            "• %APPDATA%\\ACLOS\\  （删除用户数据目录）",
        ]:
            tk.Label(detail2, text=line, font=("Consolas", FS(7)),
                     bg=COLORS['bg_alt'], fg=COLORS['fg_muted'],
                     anchor="w", padx=S(10), pady=S(3)).pack(fill=tk.X)
        ...
        hdr = tk.Frame(log_card, bg=COLORS['surface'])
        hdr.pack(fill=tk.X, padx=S(12), pady=(S(8), 0))
        tk.Label(hdr, text="操作日志", font=(F, FS(9), "bold"),
                 bg=COLORS['surface'], fg=COLORS['fg']).pack(side=tk.LEFT)
        RoundedButton(hdr, text="清空",
                      command=self._clear_log,
                      bg=COLORS['bg_alt'], fg=COLORS['fg_muted'], hover_bg=COLORS['border'],
                      font_size=FS(8), width=S(48), height=S(20), radius=S(4)).pack(side=tk.RIGHT)

        self.cache_log = _st.ScrolledText(
            log_card, font=("Consolas", FS(9)),
            bg=COLORS['bg'], fg=COLORS['fg'],
            relief=tk.FLAT, height=5, state=tk.DISABLED, padx=6, pady=3)
        self.cache_log.pack(fill=tk.BOTH, expand=True, padx=S(10), pady=(S(4), S(8)))

    def _log(self, message, level="info"):
        colors = {"info": COLORS['info'], "success": COLORS['success'],
                  "warning": COLORS['warning'], "error": COLORS['error']}
        try:
            self.cache_log.configure(state=tk.NORMAL)
            ts = time.strftime("%H:%M:%S")
            self.cache_log.insert(tk.END, f"[{ts}] ", "ts")
            self.cache_log.insert(tk.END, f"{message}\n", level)
            self.cache_log.tag_config("ts", foreground=COLORS['text_light'])
            self.cache_log.tag_config(level, foreground=colors.get(level, COLORS['fg']))
            self.cache_log.see(tk.END)
            self.cache_log.configure(state=tk.DISABLED)
        except Exception:
            pass
        self.main_app.log(f"[Cache] {message}", level)
    ...
    def clean_tool_cache(self):
        ...
        def _do():
            ...
            # ACLOS Tools 数据目录
            target_dir = DEPLOY_DIR  # %APPDATA%\ACLOS Tools
            if os.path.exists(target_dir):
                try:
                    shutil.rmtree(target_dir)
                    cleaned.append(target_dir)
                    self.parent.after(0, lambda p=target_dir: self._log(f"已删除目录: {p}", "success"))
                except Exception as e:
                    errors.append(str(e))
                    self.parent.after(0, lambda err=e: self._log(f"删除目录失败: {err}", "error"))
            else:
                self.parent.after(0, lambda: self._log(f"目录不存在，跳过: {target_dir}", "warning"))

            # asar 路径配置缓存
            if os.path.exists(PATH_CACHE):
                try:
                    os.remove(PATH_CACHE)
                    cleaned.append(PATH_CACHE)
                    self.parent.after(0, lambda p=PATH_CACHE: self._log(f"已删除文件: {p}", "success"))
                except Exception as e:
                    errors.append(str(e))
                    self.parent.after(0, lambda err=e: self._log(f"删除文件失败: {err}", "error"))
            else:
                self.parent.after(0, lambda: self._log(f"缓存文件不存在，跳过: {PATH_CACHE}", "warning"))

            # ACLOS.lnk
            desktop = get_unicode_shell_path(0x0000)
            aclos_lnk = os.path.join(desktop, "ACLOS.lnk")
            _kernel32 = ctypes.windll.kernel32
            if _kernel32.GetFileAttributesW(aclos_lnk) != 0xFFFFFFFF:
                try:
                    _kernel32.SetFileAttributesW(aclos_lnk, 0x80)
                    if not _kernel32.DeleteFileW(aclos_lnk):
                        raise ctypes.WinError()
                    cleaned.append(aclos_lnk)
                    self.parent.after(0, lambda: self._log("已删除桌面快捷方式: ACLOS.lnk", "success"))
                except Exception as e:
                    errors.append(str(e))
                    self.parent.after(0, lambda err=e: self._log(f"删除 ACLOS.lnk 失败: {err}", "error"))
            else:
                self.parent.after(0, lambda: self._log("桌面未找到 ACLOS.lnk，跳过", "info"))

            if errors:
                msg = "部分缓存清理失败：\n" + "\n".join(errors)
                self.parent.after(0, lambda: self.main_app._msgbox("error", "清理失败", msg))
            else:
                msg = f"工具缓存已全部清理完成。\n共删除 {len(cleaned)} 个目标。"
                self.parent.after(0, lambda: self._log("缓存清理完成 ✓", "success"))
                self.parent.after(0, lambda: self.main_app._msgbox("info", "清理成功", msg))
    ...
    def fresh_install(self):
        ...
        confirmed = messagebox.askyesno(
            "⚠ 危险操作确认",
            "该操作将执行以下步骤：\n\n"
            "1. 运行 ACLOS 卸载程序（uninst.exe）\n"
            "2. 强制删除 ACLOS 安装目录\n"
            "3. 删除 %APPDATA%\\ACLOS 用户数据目录\n"
            "4. 下载最新安装包并自动启动安装\n"
            "⚠ 警告：此操作将导致保存的账号密码以及\n"
            "启动器设置被永久删除，无法恢复！\n\n"
            "确认要继续执行吗？",
        ...
        if not confirmed:
            self._log("用户取消了全新安装操作", "warning")
            return

        self._log("用户确认，开始执行启动器全新安装流程...")

        def _do():
            import subprocess
            uninst_path = None
            aclos_root  = None
            _SUFFIX = os.path.join("Launcher", "resources", "app.asar")
            try:
                asar_path = None
                if os.path.exists(PATH_CACHE):
                    with open(PATH_CACHE, 'r', encoding='utf-8') as f:
                        asar_path = f.read().strip()
                    if not os.path.exists(asar_path):
                        self.parent.after(0, lambda: self._log(
                            "缓存路径已失效，重新扫描...", "warning"))
                        asar_path = None

                if asar_path is None:
                    self.parent.after(0, lambda: self._log(
                        "缓存不存在，正在重新扫描 ACLOS 目录...", "warning"))
                    asar_path = find_asar_path()

                if asar_path:
                    if asar_path.endswith(_SUFFIX):
                        aclos_root = asar_path[: -len(_SUFFIX)].rstrip(os.sep)
                    else:
                        aclos_root = os.path.dirname(os.path.dirname(os.path.dirname(asar_path)))
                    candidate = os.path.join(aclos_root, "uninst.exe")
                    if os.path.exists(candidate):
                        uninst_path = candidate
                        self.parent.after(0, lambda p=aclos_root: self._log(
                            f"定位 ACLOS 根目录: {p}", "info"))
                    else:
                        self.parent.after(0, lambda p=candidate: self._log(
                            f"uninst.exe 不存在，跳过卸载步骤: {p}", "warning"))
                else:
                    self.parent.after(0, lambda: self._log(
                        "无法定位 ACLOS 目录，跳过卸载及安装目录删除步骤", "warning"))
            except Exception as e:
                self.parent.after(0, lambda err=e: self._log(f"推导路径失败: {err}", "error"))

            if uninst_path:
                self.parent.after(0, lambda p=uninst_path: self._log(f"找到卸载程序: {p}", "info"))
                try:
                    self.parent.after(0, lambda: self._log("正在以管理员身份运行卸载程序，请等待完成...", "warning"))
                    proc = subprocess.run(
                        ["cmd", "/c", uninst_path, "/S"],
                        shell=False
                    )
                    self.parent.after(0, lambda rc=proc.returncode: self._log(
                        f"卸载程序已退出，返回码: {rc}", "success" if rc == 0 else "warning"))
                except Exception as e:
                    self.parent.after(0, lambda err=e: self._log(f"运行卸载程序失败: {err}", "error"))
            else:
                self.parent.after(0, lambda: self._log(
                    "未找到 uninst.exe，跳过卸载程序步骤", "warning"))

            errors = []
            if aclos_root and os.path.exists(aclos_root):
                self.parent.after(0, lambda p=aclos_root: self._log(f"强制删除安装目录: {p}", "warning"))
                try:
                    subprocess.run(["cmd", "/c", "rmdir", "/s", "/q", aclos_root],
                                   shell=False, timeout=30)
                    if os.path.exists(aclos_root):
                        shutil.rmtree(aclos_root, ignore_errors=True)
                    if not os.path.exists(aclos_root):
                        self.parent.after(0, lambda p=aclos_root: self._log(
                            f"安装目录已删除: {p}", "success"))
                    else:
                        self.parent.after(0, lambda p=aclos_root: self._log(
                            f"目录仍有残留，请手动删除: {p}", "warning"))
                except Exception as e:
                    errors.append(str(e))
                    self.parent.after(0, lambda err=e: self._log(f"删除安装目录失败: {err}", "error"))
            else:
                self.parent.after(0, lambda: self._log("ACLOS 安装目录不存在或已删除，跳过", "warning"))

            appdata_aclos = os.path.join(os.environ.get('APPDATA', UNICODE_APPDATA), "ACLOS")
            if os.path.exists(appdata_aclos):
                self.parent.after(0, lambda p=appdata_aclos: self._log(f"删除用户数据目录: {p}", "warning"))
                try:
                    ...
                    def _remove_readonly(func, fpath, _):
                        try:
                            ...
                            pass
                    shutil.rmtree(appdata_aclos, onerror=_remove_readonly)
                    self.parent.after(0, lambda p=appdata_aclos: self._log(
                        f"用户数据目录已删除: {p}", "success"))
                except Exception as e:
                    errors.append(str(e))
                    self.parent.after(0, lambda err=e: self._log(f"删除用户数据失败: {err}", "error"))
            else:
                self.parent.after(0, lambda: self._log(
                    f"用户数据目录不存在，跳过: {appdata_aclos}", "warning"))

            INSTALLER_URL = ("https://down.val.qq.com/aclos/release/2.13.1.393/"
                             "VALORANT_CN_installer_v2.13.1.393.exe")
            BILIBILI_URL  = "https://space.bilibili.com/321503341"
            installer_name = INSTALLER_URL.split("/")[-1]
            installer_path = os.path.join(
                os.environ.get('TEMP', os.path.expanduser('~')), installer_name)

            self.parent.after(0, lambda: self._log("开始下载安装包...", "info"))
                    ...
                    for chunk in resp.iter_content(chunk_size=1024 * 256):
                        if chunk:
                            ...
                            if total > 0:
                                ...
                                if pct >= last_pct + 5:
                                    last_pct = pct
                                    self.parent.after(0, lambda p=pct, d=downloaded, t=total:
                                        self._log(
                                            f"下载中... {p}%  "
                                            f"({d // 1024 // 1024} MB / {t // 1024 // 1024} MB)",
                                            "info"))
                download_ok = True
                self.parent.after(0, lambda: self._log(
                    f"下载完成: {installer_path}", "success"))
            except Exception as e:
                self.parent.after(0, lambda err=e: self._log(
                    f"下载失败: {err}", "error"))

            if not download_ok:
                def _on_download_fail():
                    self.main_app._msgbox(
                        "warning", "下载失败",
                        "官方下载链接失效，请从网盘下载安装包。\n"
                        "网盘链接请看 bilibili 评论区。\n\n"
                        "点击确定后将跳转到 bilibili 主页。")
                    webbrowser.open(BILIBILI_URL)
                self.parent.after(0, _on_download_fail)
                return

            self.parent.after(0, lambda: self._log(
                "正在启动安装程序，请完成安装，安装完成后不要点击“立即体验”，直接关闭安装窗口即可...", "warning"))
            try:
                import ctypes as _ct
                from ctypes import wintypes as _wt

                class SHELLEXECUTEINFO(_ct.Structure):
                    _fields_ = [
                        ("cbSize",       _ct.c_ulong),
                        ("fMask",        _ct.c_ulong),
                        ("hwnd",         _wt.HWND),
                        ("lpVerb",       _ct.c_wchar_p),
                        ("lpFile",       _ct.c_wchar_p),
                        ("lpParameters", _ct.c_wchar_p),
                        ("lpDirectory",  _ct.c_wchar_p),
                        ("nShow",        _ct.c_int),
                        ("hInstApp",     _wt.HINSTANCE),
                        ("lpIDList",     _ct.c_void_p),
                        ("lpClass",      _ct.c_wchar_p),
                        ("hkeyClass",    _wt.HKEY),
                        ("dwHotKey",     _ct.c_ulong),
                        ("hIconOrMonitor", _ct.c_void_p),
                        ("hProcess",     _wt.HANDLE),
                    ]

                SEE_MASK_NOCLOSEPROCESS = 0x00000040
                sei = SHELLEXECUTEINFO()
                sei.cbSize       = _ct.sizeof(SHELLEXECUTEINFO)
                ...
                if not ok or not sei.hProcess:
                    raise RuntimeError("ShellExecuteEx 失败，用户可能取消了 UAC")

                self.parent.after(0, lambda: self._log("安装程序已启动，等待完成...", "info"))
                _ct.windll.kernel32.WaitForSingleObject(sei.hProcess, 0xFFFFFFFF)
                _ct.windll.kernel32.CloseHandle(sei.hProcess)
                self.parent.after(0, lambda: self._log("安装程序已退出", "success"))
            except Exception as e:
                self.parent.after(0, lambda err=e: self._log(
                    f"启动安装程序失败: {err}", "error"))

            try:
                if os.path.exists(installer_path):
                    os.remove(installer_path)
                    self.parent.after(0, lambda p=installer_path: self._log(
                        f"已删除安装包缓存: {p}", "info"))
            except Exception as e:
                self.parent.after(0, lambda err=e: self._log(
                    f"删除安装包缓存失败: {err}", "warning"))

            self.parent.after(0, lambda: self._log("全新安装流程全部完成 ✓", "success"))
            if errors:
                msg = "清理阶段存在以下错误（不影响安装结果）：\n" + "\n".join(errors)
                self.parent.after(0, lambda: self.main_app._msgbox("warning", "部分清理失败", msg))
